...
void equelleGeneratedCode(equelle::EquelleRuntimeCPU& er) {
    ...
    const Scalar lambda = double(52500);
    const CollOfScalar source = er.inputCollectionOfScalar("source", er.allCells());
    ...
    const CollOfScalar k = (double(1) / ((double(1) / h1) + (double(1) / h2)));
    ...
    std::function<CollOfScalar(const CollOfScalar&, const CollOfScalar&, const Scalar&)> residual = [&](const CollOfScalar& p, const CollOfScalar& p0, const Scalar& dt) -> CollOfScalar {
        const CollOfScalar v = ((lambda * k) * (er.operatorOn(p, er.allCells(), f) - er.operatorOn(p, er.allCells(), s)));
        ...
        const CollOfScalar res = ((((vol / dt) * (rho - rho0)) + er.divergence((v * rho_face))) - source);
        return res;
    };
    const CollOfScalar p_initial = er.operatorExtend(double(1000000), er.allCells());
    CollOfScalar p0;
    p0 = p_initial;
    for (const Scalar& dt : timesteps) {
        std::function<CollOfScalar(const CollOfScalar&)> localResidual = [&](const CollOfScalar& p) -> CollOfScalar {
            return residual(p, p0, dt);
        };
        const CollOfScalar p = er.newtonSolve(localResidual, p0);
