...
public:
    ...
    ADB compute(const ADB& u, const V &u0)
    {
...
#if 0
        V fareas(num_internal);
        for (int i=0; i<num_internal; i++) {
            fareas(i) = grid_.face_areas[i];
        }

        V centroid_diff_lengths(num_internal);
        V centroid_diff_lengths_inv(num_internal);
        for (int i=0; i<num_internal; i++) {
            centroid_diff_lengths(i) = euclidean_diff(grid_.cell_centroids+i*dim, grid_.cell_centroids+(i+1)*dim, dim);
            centroid_diff_lengths_inv(i) = 1.0/euclidean_diff(grid_.cell_centroids+i*dim, grid_.cell_centroids+(i+1)*dim, dim);
        }

        const V tmp = V::Constant(num_internal, 0.0);
        ADB fluxes = ADB::variable(0, tmp, { num_internal } ); // (block index, initialized from, and block structure)
        for (int i=0; i<num_internal; i++) {
            // fluxes(i) = k_ * fareas(i) * (u_first(i)-u_second(i)) / centroid_diff_lengths(i);
            // This doesn't compile. Is it menaingless? Just not implemented? Plain wrong?!
        }
        ...
        // @jny Maybe too much meaning is implicitly put into "diff(centroid(...))" here, since "diff(centroid)" has to be understood as
        // "pair_of_centroids(face)". Would "centroid_pair(faces(grid))" be better than "centroid(cells(grid))"? If so, we are almost at alternative 6 already.
        // Both "alt. 5 Equelle" and "alt. 6 Equelle" versions could translate to the the "alt. 6 C++" code snippet, but for the sake of the example, here is a
        // more detailed loop that the Equelle code also could be producing
        // (Since this ignores the grid-contained topology, it's maybe not a good idea.
        // Writing out the loop using ops_.ngrad will just be duplicating the matrix-multiplication in the "alt. 6" version...)
        V fareas(num_internal);
        for (int i=0; i<num_internal; i++) {
            fareas(i) = grid_.face_areas[i];
        }
        ...
        // --------------------------------------------------------------------------------
        // A bit more true to the Equelle-code:
        const M first_faces  =   pick_elements(ops_.ngrad, 1);
        const M second_faces =   pick_elements(ops_.ngrad, -1);
        const ADB u_first  = first_faces  * u;
        const ADB u_second = second_faces * u;
        const ADB fluxes = trans * (u_first + u_second); // Note the sign.

#endif

        // --------------------------------------------------------------------------------
        // Alt. 5: vol : scalar(cells(grid)) = volume(cells(grid))
        // Alt. 6: vol : scalar(cells(grid)) = volume
        // --------------------------------------------------------------------------------
        // @jny Little sense in different C++ versions of these Equelle-lines?
...
int main(int argc, char** argv)
{
    ...
    // Create a 1D Cartesian grid.
    Opm::GridManager gm(n, 1); // @jny The data structure seems not to support 1D grids directly
