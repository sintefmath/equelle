#!/usr/bin/python
# ---- This script generates a JavaScript function which mathes an input stream with regular expressions, and returns a token ----
...
import fileinput
# Read the equelle_lexer.y from the stdin and parse
section = 'definitions'
comment = False
...
import sys
# All regexes and rules are read, create the JavaScript lexer function

# Read the skeleton from lexerSkel.js
skel = open('lexerSkel.js','r')
for line in skel:
    ir = line.find('##regexes##');
    ia = line.find('##actions##');
    # Insert the regexes into the skeleton, with indentation, so that it looks nice
    if (ir >= 0):
        separator = ' '
        indent = ' '*ir
        for rule in rules:
